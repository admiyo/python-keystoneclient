...
class Discover(object):
    ...
    def raw_version_data(self, allow_experimental=False,
        ...
        for v in self._data:
            ...
            if status in self.CURRENT_STATUSES:
                versions.append(v)
            elif allow_deprecated and status in self.DEPRECATED_STATUSES:
                versions.append(v)
            elif allow_experimental and status in self.EXPERIMENTAL_STATUSES:
                versions.append(v)
            elif allow_unknown:
                versions.append(v)
    ...
    def version_data(self, **kwargs):
        ...
        Return version data in a structured way.  the server
        ...
        :returns list(dict): A list of version data dictionaries sorted by
          ...
          :raw_status str: The status as provided by
        """
    ...
    def data_for(self, version, **kwargs):
        ...
        :returns dict: the endpoint data for a URL that matches the required
                       version (The format is descrbed in version_data)
                       or None if no match.
        """
